...
def is_processed(obj_name):
    sql = "select file_name from ho_production.loading_to_hdfs_log where file_name = '" + obj_name + "'"
    cursor.execute(sql)
...
def get_the_latest_modified_date():
    sql = "select max(last_modified_at) as latest_modified_date from ho_production.loading_to_hdfs_log"
    print sql
    cursor.execute(sql)
...
def save_the_latest_modified_date(object_name, date_str):
    sql = "insert into ho_production.loading_to_hdfs_log (file_name, last_modified_at) values " \
...
def download_s3_obj_and_sync_to_hdfs(file_name):
    ...
    temp_str = file_name.split('/E3keQDgoSf/')
...
if __name__ == '__main__':
    AWS_KEY = 'xxxxx'
    AWS_SECRET = 'xxxxx'
    #raw_data_folder = '/Users/jerrycheng/PycharmProjects/segment_data/raw_data/'
    raw_data_folder = '/home/hduser/raw_data/segment/ho_production/'
    hdfs_path = '/segment/ho_production'
